...
<body>
	<h2>
	<input class = "nextButton" type="button" onclick="location.href='prCal.do?i=m'" value="이전 달">
	${year}년 ${month}월 캘린더
	<input class = "nextButton" type="button" onclick="location.href='prCal.do?i=p'" value="다음 달">
				...
				for(int i = 0; i<5 ;i++)
				{
					...
					{ for(int t = 0; t<fD-1 ;t++)
						...
						{ 
							...
							<td id = "today"><a href="prDaySession.jsp?day=<%=day %>"><%=day %></a>
							<ul><%for(int p = 0; p<prMemos.size() ;p++) {
								PrMemo prMemo = prMemos.get(p);
								if(Integer.parseInt(prMemo.getDay())==day){
									%><li><%="-"+prMemo.getPrMemo() %></li>
								<%}
							}day++;%>
							</ul></td>
						<%} else {
							%>
							<td><a href="prDaySession.jsp?day=<%=day %>"><%=day %></a>
							<ul><%for(int p = 0; p<prMemos.size() ;p++) {
								PrMemo prMemo = prMemos.get(p);
								if(Integer.parseInt(prMemo.getDay())==day){
									%><li><%="-"+prMemo.getPrMemo() %></li>
